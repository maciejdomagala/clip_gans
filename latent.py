...
class DeepMindBigGANLatentSpace(torch.nn.Module):
    def __init__(self, config):
        ...
        self.z = torch.nn.Parameter(torch.tensor(truncated_noise_sample(
            self.config.batch_size)).to(self.config.device))
        self.class_labels = torch.nn.Parameter(torch.rand(
            self.config.batch_size, self.config.num_classes).to(self.config.device))
    ...
    def set_from_population(self, x):
        self.z.data = torch.tensor(x[:, :self.config.dim_z].astype(
            float)).float().to(self.config.device)
        self.class_labels.data = torch.tensor(
            x[:, self.config.dim_z:].astype(float)).float().to(self.config.device)
...
class StyleGAN2LatentSpace(torch.nn.Module):
    def __init__(self, config):
        ...
        self.z = torch.nn.Parameter(torch.randn(
            self.config.batch_size, self.config.dim_z).to(self.config.device))

    def set_values(self, z):
        self.z.data = z

    def set_from_population(self, x):
        self.z.data = torch.tensor(
            x.astype(float)).float().to(self.config.device)
