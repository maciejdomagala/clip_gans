...
\noindent One popular challenge in computer vision field is the text-to-image modelling problem, which can be currently described as highly experimental.  Recently, the OpenAI company released the results of their work on a new model to solve this problem called \textit{DALL-E} \cite{dalle}. According to the authors DALL-E is a 12-billion parameters model which "has a diverse set of capabilities, including creating anthropomorphized versions of animals and objects, combining unrelated concepts in plausible ways, rendering text, and applying transformations to existing images" \cite{dalle-blog}.  The appearance OpenAI's paper was our motivation to create the following diploma thesis. Second aspect that convinced us to bring up this topic was the fact that the text-to-image problem allows us to tackle different areas of ML - both NLP and Computer vision. \\
The DALL-E model consists of two parts - the first part is responsible for generating images, while the second part, called \textit{CLIP} is responsible for answering the question how the generated image complies with the given description.  So far, OpenAI has made only CLIP model available online.  For this reason, in the project described below, we decided to find and test other models that could replace the generative part of the DALL-e model, while realizing that we would probably find much smaller models and our target solution would not be able to generalize the problem as well as DALL-E.  We decided to limit our selection to a group of popular generative models such as Generative Adversarial Models (GANs).  Given the subjective nature of GAN models, need for empirical validation of results and lack of all-round metrics for proper evaluation, there are currently no useful methods of comparing different approaches. We address this issue by performing evaluation on different datasets with use of several popular image classifiers. We also show that it is possible to navigate the latent space with evolutionary algorithms to obtain a good quality image representation from text.

\noindent This paper was written by the pair Maciej Domagała and Adam Komorowski.  We divided the work on this project as follows - Maciej was responsible for describing the evolutionary algorithms (1.3), the StyleGAN and BigGAN models (2.1, 2.2, 2.3), our framework (3.1, 3.2) and conducting and describing some of the experiments including the comparison of StyleGAN vs BigGAN models (4.2) and the comparison of results for different evolutionary algorithms (4.3).  Adam was responsible for preparing the notebooks to conduct the experiments in the project and for describing the GAN models (1.1), the CLIP model (1.2), the section on sample results and observations from the experiments (4.1) and model evaluations for the CIFAR10 and ImageNet datasets (5.1, 5.2).
...
\noindent According to CLIP's authors \cite{clip_blog}- the model has several limitations. These include:
\begin{itemize}
...
\noindent The main advantages over general genetic algorithm is more efficient memory usage, lower complexity and faster convergence.
 ...
 \begin{figure}[ht!]
     ...
     \caption{Differential evolution algorithm pseudocode.}\label{Fig:PROGAN}
 \end{figure}
...
\noindent In the traditional generator, latent code is introduced to the network only in the first layer. The authors of StyleGAN are using the vector $w$ produced via mapping network to steer the style of an image at each convolutional layer by using \textbf{Adaptive Instance Normalization (AdaIN)} technique.
...
\noindent The middle part of the equation is the \textbf{Instance Normalization (IN)}. Each channel of the convolution layer is normalized. Values $\textbf{y}_{s,i}$ and $\textbf{y}_{b,i}$ are calculated from the vector $v$ using fully-connected layer and correspond to \textit{A} on the figure-main one. These can be understood as scale and bias; these values are used to translate the information from vector $w$ to a feature map generated by convolution.
...
\caption[pics]{Comparison of StyleGAN2-cat, StyleGAN2-ca and BigGAN for "a clown cyclist on the moon" phrase.}
\end{figure}
...
\bibitem[1]{gan} I. Goodfellow et al.  {\it Generative Adversarial Nets} 2014.  \url{https://arxiv.org/pdf/1406.2661.pdf}

\bibitem[2]{ganprogress} M. Brundage, S. Avin, J. Clark et al. {\it The Malicious Use of Artificial Intelligence: Forecasting, Prevention, and Mitigation.} arXiv preprint arXiv: 1802.07228, 2018.

\bibitem[3]{gan_arch} \url{https://machinelearningmastery.com/what-are-generative-\\adversarial-networks-gans/}

\bibitem[4]{improvedgans} T. Salimans, I. Goodfellow, W. Zaremba, V. Cheung, A. Radford, and X. Chen. {\it Improved techniques for training gans.} In NIPS, 2016.

\bibitem[5]{inception} C. Szegedy, V. Vanhoucke, S. Ioffe, J. Shlens, Z. Wojna. {\it Rethinking the inception architecture for computer vision.} arXiv preprint arXiv: 1512.00567, 2015.

\bibitem[6]{fid} M. Heusel, H. Ramsauer, T. Unterthiner, B. Nessler, S. Hochreiter. {\it GANs trained by a two time-scale update rule converge to a local Nash equilibrium.} In NIPS, 2017.

\bibitem[7]{clip} OpenAI CLIP \url{https://arxiv.org/pdf/2103.00020.pdf}

\bibitem[8]{clip_blog} \url{https://openai.com/blog/clip/}

\bibitem[9]{clip_source} \url{https://habr.com/en/post/537334/}

\bibitem[10]{de} R. Storn, K. Price. {\it Differential Evolution – A Simple and Efficient Heuristic for global Optimization over Continuous Spaces.} Springer, 1997.

\bibitem[11]{stylegan} T. Karras, S. Laine, T. Aila. {\it A style-based generator architecture for generative adversarial networks.} In CVPR, 2019.

\bibitem[12]{biggan} A. Brock, J. Donahue, K. Simonyan {\it Large Scale GAN Training for High Fidelity Natural Image Synthesis.} arXiv preprint arXiv: 1809.11096, 2019.

\bibitem[13]{aphantasia} \url{https://github.com/eps696/aphantasia}
...
\bibitem[17]{bigsleep} \url{https://github.com/lucidrains/big-sleep}

\bibitem[18]{deepai} \url{https://deepai.org/machine-learning-model/text2img}

\bibitem[19]{pymoo} \url{https://github.com/msu-coinlab/pymoo}

\bibitem[20]{cifar10_data} \url{https://www.cs.toronto.edu/~kriz/cifar.html}

\bibitem[21]{imagenet_example} \url{https://devopedia.org/imagenet}

\bibitem[22]{imagenet_data} \url{https://www.image-net.org/}
...
\bibitem[25]{hand} https://arxiv.org/pdf/2009.00678.pdf

\bibitem[26]{superres} https://arxiv.org/pdf/1609.04802.pdf

\bibitem[27]{blend} https://arxiv.org/pdf/1703.07195.pdf

\bibitem[28]{ea} https://arxiv.org/pdf/1906.08870.pdf

\bibitem[29]{progan} https://arxiv.org/pdf/1710.10196.pdf

\bibitem[30]{proganvsstylegan} https://medium.com/gradientcrescent/this-president-does-not-exist-generating-artistic-portraits-of-donald-trump-using-stylegan-a97a17902dd4

\bibitem[31]{clipresults} https://github.com/lucidrains/DALLE-pytorch/discussions/131

\bibitem[32]{dalle-blog} https://openai.com/blog/dall-e/

\bibitem[33]{dalle} https://arxiv.org/pdf/2102.12092.pdf
\end{thebibliography}
